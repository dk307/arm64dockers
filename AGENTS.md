...
├── .github/
│   └── workflows/
│       ├── release-monitor.yml          # Daily cron: checks upstream releases, triggers builds
...
**Awaiting build instructions from user.**
...
| Tag | Behavior |
|-----|----------|
| `:bNNNN` | Specific upstream release (overwrites on rebuild) |
...
2. **Release tags only** — never master/main/dev. Enforced by `^b[0-9]+$` regex in workflow.
3. **Single repo** — all containers in this one repository.
4. **Public GHCR** — no auth needed for pulls.
5. **No default tag** — `ARG LLAMA_CPP_TAG` has no default. Build fails if not passed.
